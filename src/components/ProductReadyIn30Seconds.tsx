...
export const ProductReadyIn30Seconds: React.FC<ProductReadyIn30SecondsProps> = ({
  ...
  const handleFinalPublish = () => {
    ...
    const finalProduct: Product = {
      ...
      price: Number(editablePrice) || 500,
      oldPrice: Math.round((Number(editablePrice) || 500) * 1.25),
      suggestedPrice: Number(generatedProduct.suggestedPrice) || Number(editablePrice) || 500,
      minPrice: Number(generatedProduct.minPrice) || Math.round((Number(editablePrice) || 500) * 0.75),
      maxPrice: Number(generatedProduct.maxPrice) || Math.round((Number(editablePrice) || 500) * 1.3),
      quantity: Number(generatedProduct.quantity) || 5,
      status: 'published',
      emoji: generatedProduct.emoji || '🪷',
      views: 0,
      sold: 0,
      imageOriginal: originalPhoto,
      imageEnhanced: enhancedPhoto || originalPhoto || 'https://images.unsplash.com/photo-1606744824163-985d376605aa?auto=format&fit=crop&w=800',
      enhanceMode: 'studio',
      ...
      createdAt: Date.now(),
      images: [enhancedPhoto || originalPhoto || 'https://images.unsplash.com/photo-1606744824163-985d376605aa?auto=format&fit=crop&w=800'],
    };
