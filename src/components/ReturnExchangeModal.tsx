        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div className="p-3.5 bg-[#FAF9F7] rounded-2xl border border-[#E6D5C3] space-y-1 text-xs">
            ...
            <p className="text-[#6D5843] pt-1">
              <strong>{order.shippingDetails?.fullName || 'Valued Customer'}</strong> ({order.shippingDetails?.phone || 'N/A'})
            </p>
            <p className="text-[#8C7355] text-[11px]">
              {order.shippingDetails?.addressLine1 || 'Registered Address'}, {order.shippingDetails?.city || ''},{' '}
              {order.shippingDetails?.state || ''}{order.shippingDetails?.pincode ? ` - ${order.shippingDetails.pincode}` : ''}
            </p>
          </div>
