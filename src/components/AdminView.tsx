            ...
            <tbody className="divide-y divide-[#E6D5C3]">
              {filteredOrders.map((o) => (
                <tr key={o.id} className="hover:bg-[#FAF9F7]/70 transition-colors">
                  ...
                  <td className="py-3">
                    <p className="font-bold text-[#3E2723]">{o.shippingDetails?.fullName || 'Valued Customer'}</p>
                    <p className="text-[10px] text-[#8C7355]">
                      {o.shippingDetails?.city || ''}{o.shippingDetails?.state ? `, ${o.shippingDetails.state}` : ''} ({o.shippingDetails?.phone || 'N/A'})
                    </p>
                  </td>
                  <td className="py-3 text-[#8B5E34] font-medium">
                    {(o.items || []).map((i) => i.artisanName).join(', ') || 'Artisan Partner'}
                  </td>
                  <td className="py-3 font-bold text-[#8B5E34]">₹{(Number(o.totalAmount) || 0).toLocaleString('en-IN')}</td>
                  <td className="py-3">
