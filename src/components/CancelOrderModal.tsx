...
export const CancelOrderModal: React.FC<CancelOrderModalProps> = ({
  ...
  return (
    <div
      onClick={onClose}
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#3E2723]/60 backdrop-blur-xs"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-lg bg-white border border-[#E6D5C3] rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200"
      >
        {/* Modal Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#8C7355]">
                {t.orderId}: #{order.trackingId || order.id}
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close / Exit cancel dialog"
            title={language === 'hi' ? 'बाहर निकलें' : 'Exit'}
            className="p-2 text-[#8C7355] hover:text-[#3E2723] hover:bg-[#F5F1EE] rounded-xl transition-colors cursor-pointer flex items-center gap-1"
          >
            <span className="text-xs font-semibold">{language === 'hi' ? 'बाहर निकलें' : 'Exit'}</span>
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="p-4 bg-[#FAF9F7] rounded-2xl border border-[#E6D5C3] space-y-2.5">
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-semibold text-[#3E2723] flex items-center gap-1">
                ...
                {String(order.paymentMethod || '').includes('UPI')
                  ? language === 'hi'
                    ? 'मूल UPI खाता (तत्काल)'
