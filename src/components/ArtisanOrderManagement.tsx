...
export const ArtisanOrderManagement: React.FC<ArtisanOrderManagementProps> = ({
  ...
  const relevantOrders = orders.filter((o) =>
    (o.items || []).some(
      (item) =>
        item.artisanId === profile.id ||
        item.artisanName === profile.name ||
        products.some((p) => p.id === item.productId && (p.artisanId === profile.id || p.artisanName === profile.name))
    )
  );
                ...
                <div className="space-y-2.5 p-4 bg-white border border-[#E6D5C3] rounded-2xl shadow-xs">
                  ...
                  <div className="text-xs space-y-1.5 text-[#3E2723]">
                    <div className="flex items-center gap-2">
                      ...
                      <span>Customer: <strong className="text-sm">{order.shippingDetails?.fullName || 'Valued Customer'}</strong></span>
                    </div>

                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Phone className="w-3.5 h-3.5 text-[#8C7355]" />
                        <span>Contact: <strong>{order.shippingDetails?.phone || 'N/A'}</strong></span>
                      </div>
                      {order.shippingDetails?.phone && (
                        <a
                          href={`https://wa.me/${order.shippingDetails.phone.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Namaste ${order.shippingDetails.fullName || 'Customer'}, this is ${profile.name} from KalaKriti regarding your order #${order.trackingId}.`)}`}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    <div className="p-2.5 bg-[#FAF9F7] rounded-xl border border-[#E6D5C3] text-xs leading-relaxed">
                      <p className="font-semibold text-[#8B5E34]">{order.shippingDetails?.addressLine1 || 'Delivery Address'}</p>
                      {order.shippingDetails?.addressLine2 && <p>{order.shippingDetails.addressLine2}</p>}
                      <p className="font-bold text-[#3E2723] mt-0.5">
                        {order.shippingDetails?.city || ''}{order.shippingDetails?.state ? `, ${order.shippingDetails.state}` : ''}{order.shippingDetails?.pincode ? ` — PIN: ${order.shippingDetails.pincode}` : ''}
                      </p>
          ...
          <div className="w-full max-w-lg bg-white border border-[#E6D5C3] rounded-3xl shadow-2xl overflow-hidden p-6 space-y-4">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="p-3 bg-[#FAF9F7] rounded-xl text-[11px] text-[#8C7355] border border-[#E6D5C3]">
                <strong className="text-[#3E2723]">Recipient:</strong> {dispatchModalOrder.shippingDetails?.fullName || 'Customer'} — {dispatchModalOrder.shippingDetails?.city || ''}, {dispatchModalOrder.shippingDetails?.state || ''} (PIN: {dispatchModalOrder.shippingDetails?.pincode || 'N/A'})
              </div>
            </div>
            ...
            <div className="p-5 border-2 border-dashed border-[#E6D5C3] rounded-2xl bg-[#FAF9F7] space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-bold text-sm text-[#3E2723]">{selectedOrder.shippingDetails?.fullName || 'Customer'}</p>
                  <p>{selectedOrder.shippingDetails?.addressLine1 || 'Delivery Address'}</p>
                  {selectedOrder.shippingDetails?.addressLine2 && <p>{selectedOrder.shippingDetails.addressLine2}</p>}
                  <p className="font-bold">{selectedOrder.shippingDetails?.city || ''}{selectedOrder.shippingDetails?.state ? `, ${selectedOrder.shippingDetails.state}` : ''}{selectedOrder.shippingDetails?.pincode ? ` - ${selectedOrder.shippingDetails.pincode}` : ''}</p>
                  <p>Ph: {selectedOrder.shippingDetails?.phone || 'N/A'}</p>
                </div>
              </div>
              ...
              <div className="border-t border-[#E6D5C3] pt-2 space-y-1">
                ...
                {(selectedOrder.items || []).map((i, idx) => (
                  <p key={idx}>• {i.name} (Qty: {i.qty}) — ₹{(Number(i.price) || 0) * (Number(i.qty) || 1)}</p>
                ))}
              </div>
            </div>
