...
export const AddProductWizard: React.FC<AddProductWizardProps> = ({
  ...
  try {
    // 1. Find or fallback the selected category in Supabase
    ...
    try {
      ...
      console.warn('Could not query category from database, using fallback:', catErr);
    }

    // 2. Insert the product into Supabase (if available)
    let savedProductId = `prod_${Date.now()}_${Math.floor(Math.random() * 10000)}`;
    try {
      ...
      console.warn('Database offline or unreachable, continuing with local persistence:', saveErr);
    }

    // 3. Create the Product object your existing website expects
    const newProduct: Product = {
      ...
      oldPrice: Math.round((Number(price) || 500) * 1.25),
      suggestedPrice: Number(suggestedPrice) || Number(price) || 500,
      minPrice: Number(minPrice) || 300,
      maxPrice: Number(maxPrice) || 1200,
      ...
      imageEnhanced: enhancedPhoto || originalPhoto || 'https://images.unsplash.com/photo-1606744824163-985d376605aa?auto=format&fit=crop&q=80&w=800',
      enhanceMode,
      artisanId: artisanProfile?.id || 'artisan_default',
      artisanName: artisanProfile?.name || 'Master Artisan',
      artisanLocation: artisanProfile?.location || 'India',
      createdAt: Date.now(),
      images: [enhancedPhoto || originalPhoto || 'https://images.unsplash.com/photo-1606744824163-985d376605aa?auto=format&fit=crop&q=80&w=800'],
    };
