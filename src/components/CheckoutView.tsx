...
export const CheckoutView: React.FC<CheckoutViewProps> = ({
  ...
  const [shipping, setShipping] = useState<ShippingDetails>(() => ({
    ...
    pincode: defaultShipping?.pincode || '',
    landmark: defaultShipping?.landmark || '',
    deliveryNotes: defaultShipping?.deliveryNotes || '',
  }));
  ...
  // Safeguard items array against null/undefined or corrupted records
  const validItems = (Array.isArray(items) ? items : []).filter(
    (item): item is CartItem => Boolean(item && item.product)
  );
  ...
  const firstArtisanName = validItems[0]?.product?.artisanName || 'KalaKriti Artisan Collective';
  const payeeUpi = firstArtisanName
    ? `${firstArtisanName.toLowerCase().replace(/[^a-z0-9]/g, '') || 'kalakriti'}@upi`
    : 'kalakriti.artisan@okhdfcbank';
  const payeeName = firstArtisanName;

  // Generate a stable simulated order ID for this checkout session
  const [tempOrderId] = useState(() => 'KK' + Math.floor(Math.random() * 900000 + 100000));
  ...
  const handleInitiatePayment = () => {
    if (validItems.length === 0) {
      setErrorMsg(
        language === 'hi'
          ? 'आपकी टोकरी खाली है।'
          : 'Cannot checkout with an empty bag.'
      );
      return;
    }
    if (
    ...
    try {
      ...
      const newOrder: Order = {
        ...
        items: validItems.map((i) => ({
          productId: i.product.id || 'prod_' + Math.random().toString(36).substring(2, 8),
          name: i.product.name || 'Handcrafted Artisan Item',
          price: Number(i.product.price) || 0,
          qty: Number(i.quantity) || 1,
          artisanId: i.product.artisanId || 'artisan_1',
          artisanName: i.product.artisanName || 'Master Artisan',
          category: i.product.category || 'Home Decor',
          image: i.product.imageEnhanced || i.product.imageOriginal || undefined,
          emoji: i.product.emoji || '🪷',
        })),
        shippingDetails: {
          fullName: shipping?.fullName || 'Customer',
          phone: shipping?.phone || '',
          email: shipping?.email || '',
          addressLine1: shipping?.addressLine1 || '',
          addressLine2: shipping?.addressLine2 || '',
          city: shipping?.city || 'India',
        ...
        transactionId: transactionRef || (paymentMethod === 'COD' ? 'COD_PENDING' : 'REF_OK'),
        status: 'Order Placed',
        ...
        trackingTimeline: [
          {
            ...
            location: `${shipping?.city || 'Central'} Hub`,
      ...
      setErrorMsg(
        language === 'hi'
          ? 'ऑर्डर दर्ज करने में समस्या आई। कृपया पुनः प्रयास करें।'
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      {/* Top Header Navigation Bar with Exit Button */}
      <div className="flex items-center justify-between">
        <button
          type="button"
          onClick={onBackToCart}
          className="flex items-center gap-1.5 text-xs font-bold text-[#8B5E34] hover:underline cursor-pointer"
        >
        ...
        <button
          ...
          className="p-2 text-[#8C7355] hover:text-[#3E2723] hover:bg-[#F5F1EE] rounded-xl transition-colors cursor-pointer flex items-center gap-1 text-xs font-semibold"
          title={language === 'hi' ? 'चेकआउट से बाहर निकलें' : 'Exit Checkout'}
        >
          <span>{language === 'hi' ? 'बाहर निकलें' : 'Exit'}</span>
          <X className="w-4 h-4" />
        </button>
      </div>
            ...
            <div className="space-y-3">
              ...
              >
                <input
                  ...
                  className="mt-1 accent-[#8B5E34] cursor-pointer"
                />
                ...
                <div key={item.product?.id || idx} className="flex items-center gap-3 pt-3 first:pt-0">
                  <div className="w-10 h-10 rounded-xl bg-[#F5F1EE] border border-[#E6D5C3] flex items-center justify-center shrink-0">
                    {item.product?.imageEnhanced ? (
                      ...
                      />
                    ) : item.product?.imageOriginal ? (
                      <img
                        src={item.product.imageOriginal}
                        alt={item.product.name || 'Product'}
                        className="w-full h-full object-cover rounded-xl"
                      />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-[#3E2723] truncate">
                      {item.product?.name || 'Handmade Craft'}
                    </p>
                    <p className="text-[10px] text-[#8C7355]">
                      {language === 'hi' ? 'कारीगर' : 'By'} {item.product?.artisanName || 'Artisan'}
                    </p>
                    <p className="text-xs font-semibold font-serif text-[#8B5E34]">
                      ₹{(Number(item.product?.price) || 0).toLocaleString('en-IN')} × {item.quantity || 1}
                    </p>
                  </div>
                </div>
              ))}
